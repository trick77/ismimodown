// stream.ts — a generic Server-Sent Events consumer, ported from peeq (which
// took it from loom). fetch + ReadableStream rather than EventSource, so the
// caller gets a real AbortSignal and the reconnect policy stays ours.
//
// Callers receive a raw (event, data) pair and decode the payload themselves,
// so an unrecognised frame — notably the server's `: keepalive` comment, which
