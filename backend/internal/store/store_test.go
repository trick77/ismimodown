...
func TestSchemaTablesExistAndAreStrict(t *testing.T) {
	...
	// skipped_runs was here until 0005 dropped it.
...
func TestInferProbeRequiresACycle(t *testing.T) {
	...
	_, err := db.Exec(
		`INSERT INTO infer_probes (cycle_id, model_id, ok) VALUES (999, 'mimo-v2.6-flash', 1)`)
	if err == nil {
...
func TestDeletingACycleCascades(t *testing.T) {
	...
	for _, stmt := range []struct {
		...
		{`INSERT INTO infer_probes (cycle_id, model_id, ttft_ms, ok, answer_ok) VALUES (?, 'mimo-v2.6-flash', 912.0, 1, 1)`, []any{id}},
...
func TestCheckConstraintsRejectUnknownEnums(t *testing.T) {
	...
	cases := []struct {
		...
		{"out-of-range ok flag", `INSERT INTO infer_probes (cycle_id, model_id, ok) VALUES (?, 'mimo-v2.6-flash', 2)`},
